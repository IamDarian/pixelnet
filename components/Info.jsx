"use client";
import Image from "next/image";
import { Oswald } from "next/font/google";
import calendar from "@/public/svg/calendar.svg";
import phone from "@/public/svg/phone.svg";
import location from "@/public/svg/location.svg";
import EmbededMap from "./Map/EmbeddedMap";
// import Maps from "@/components/Map/Maps";

const oswald = Oswald({ subsets: ["latin"] });

export default function Info() {
  return (
    <section className="bg-black py-14 px-4">
      <div className="container mx-auto flex justify-center">
        <div className="flex lg:flex-row flex-col lg:w-full w-auto">
          <div className="flex flex-col justify-center bg-[#404040] border border-solid border-secondary p-10">
            <h3 className={`${oswald.className} text-3xl font-semibold mb-2`}>
              Schedule
            </h3>
            <div className="flex items-center mb-2">
              <Image src={calendar} alt="Calendar Icon" />
              <div className="flex flex-col ml-4">
                <p className="md:inline flex flex-col">
                  Monday-Friday: <span>14:00-22:00</span>
                </p>
                <p className="md:inline flex flex-col">
                  Saturady-Sunday: <span>12:00-00:00</span>
                </p>
              </div>
            </div>
            <h3
              className={`${oswald.className} text-3xl font-semibold mt-4 mb-2`}
            >
              Adress
            </h3>
            <div className="flex items-center mb-2">
              <Image src={location} alt="Location Icon" />
              <p className="ml-4">Tractorul, Brasov, Romania</p>
            </div>
            <div className="flex items-center">
              <Image src={phone} alt="Phone Icon" />
              <p className="lg:inline flex flex-col ml-4">
                Phone for reservations: <span>+40712 345 678</span>
              </p>
            </div>
          </div>
          <div className="bg-[#404040] h-full grow">
            <EmbededMap />
            {/* {window === undefined ? (
              <p>Loading...</p>
            ) : (
              <Maps latitude={45.6669099} longitude={25.6078415} />
            )} */}
          </div>
        </div>
      </div>
    </section>
  );
}
